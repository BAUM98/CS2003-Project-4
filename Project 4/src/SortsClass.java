   // This code will compile with warnings about unchecked exceptions
	 ...
	 public class SortsClass {
      ...
      // copy the result back into the original array
       ...
       int first, int last, int insertion) {
      // ---------------------------------------------------------
      // Sorts the items in an array into ascending order.
      ...
      // Calls: merge.
      // ---------------------------------------------------------
         if (last - first > insertion) {
         // sort each half
            int mid = (first + last)/2; // index of midpoint
         // sort left half theArray[first..mid]
            mergesort(theArray, first, mid, insertion);
         // sort right half theArray[mid+1..last]
            mergesort(theArray, mid+1, last, insertion);
         // merge the two halves
            if(insertion != 1) {
            	Comparable[] tempArray = new Comparable[last - first + 1];
            for(int i = first, j = 0; i <= last; i++, j++) {
            	tempArray[j] = theArray[i];
            }
            insertionSort(tempArray, tempArray.length);
            for(int i = first, j = 0; i <= last; i++, j++)
            	theArray[i] = tempArray[j];
            }
            else
